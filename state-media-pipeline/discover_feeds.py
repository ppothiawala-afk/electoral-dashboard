...
Two modes:
  ...
  --health-check      Re-validate feeds ALREADY in the registry. Feeds that
                      ...
                      are emitted to feeds_patch.json as "flag" proposals
                      (status -> flagged/dead). Nothing is removed automatically.
...
def main():
    ...
    print(f"proposals -> {PATCH.name}: "
          ...
          f"{len(patch['rejects'])} rejects (review before applying)")
